...
这一版重点解决 Windows 桌面版打开后唤醒独立显卡、造成风扇升速或线圈声音的问题：

- 关闭界面硬件加速，图表改用轻量软件渲染，不再为了显示看板持续占用 NVIDIA 独显。
- 窗口隐藏到系统托盘后停止曲线重绘，但后台采集、历史保存和手机实时访问继续运行。
- 默认采样间隔从 5 秒调整为 10 秒，并可在“使用设置”中选择 5、10 或 30 秒。
- 去除大面积实时模糊效果，保留渐变、层次和交互读数，同时降低持续渲染开销。
- 设置页新增 PulseBoard 自身 CPU 与内存占用，方便直接判断监控工具是否影响电脑。
- 不同采样频率会写入历史记录，切换到 30 秒后也不会被错误标记为离线。

![PulseBoard v2.3 低功耗运行设置](docs/images/windows-low-power-v2.3.png)

> 在 RTX 3090 测试机上，旧版 PulseBoard 会出现在 NVIDIA 活动进程中；2.3 运行时已不再占用 NVIDIA 独显。实测窗口显示约占 0.106% CPU，隐藏到托盘约占 0.057% CPU，具体数值会随电脑配置变化。

## v2.2.0：自动记录与离线区间

这一版重点解决长时间运行时的数据完整性：

- Windows 开机后自动在系统托盘启动并继续记录，不再依赖手动打开。
- 电脑关机、休眠或 PulseBoard 未运行超过 20 秒时，曲线会按真实时间显示灰色“离线”区间。
- 离线期间 CPU、内存、GPU、显存、磁盘和网络统一记为 `0`，不会再把关机前后的曲线直接连起来。
- 新增连续记录时长、离线时长、有效数据点和开机自启动状态概览。
- 新增当前时间范围 CSV 导出，方便用 Excel 分析。
- 增加单实例保护，重复打开软件不会产生两份采集进程或重复记录。
- 调整卡片、状态栏、间距、颜色和曲线时间轴，信息层次更清楚。

![PulseBoard v2.2 离线区间与记录概览](docs/images/windows-offline-history-v2.2.png)

> 点击右上角“使用设置”可以随时开启或关闭开机自启动。安装版和便携版都支持；如果移动了便携版文件，请重新打开一次软件以更新启动路径。
...
| 能力 | 说明 |
| --- | --- |
| 📊 实时资源看板 | CPU、内存、GPU、显存、磁盘读写和网络流量按 5、10 或 30 秒刷新 |
| 🕒 长期历史曲线 | 支持回看最近 1 小时、6 小时、24 小时、7 天和 30 天 |
| ⏻ 离线区间 | 关机、休眠或程序中断会在真实时间轴上显示为灰色区域，各项资源归零 |
...
| 🚀 开机自动记录 | 随 Windows 静默启动到系统托盘，也可在“使用设置”中关闭 |
| 📤 CSV 导出 | 导出当前选择的时间范围，包含在线/离线状态和各项资源数值 |
...
3. 点击“使用设置”确认开机自启动，并根据需要选择 5、10 或 30 秒采样间隔；推荐保持 10 秒。
4. 关闭窗口时，PulseBoard 会缩到系统托盘并继续记录；在托盘菜单中选择“退出”才会完全停止。
5. 需要进一步分析时，点击“导出 CSV”保存当前时间范围的数据。
